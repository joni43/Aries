...
function printdoc() {
  ...
  let WinPrint = window.open(
    "",
    "",
    "letf=300,top=300,width=461,height=341,toolbar=110,scrollbars=30,status=0"
  );
...
window.onload = function() {
  ...
  window.addEventListener("click", function(event) {
    ...
    if (event.target.className === "far fa-star") {
      ...
      event.target.parentElement.className === "document-handler-item"
    ) {
  ...
  //  }
    ...
    console.log("click detected, noteToView: " + noteToView);
    if (noteToView) {
      updateNote(noteToView);
    } else {
      saveNote();
